...
# 4 (HOVER): Ciano ('cyan') - Célula diretamente sob o cursor do mouse.
...
def toggle_connections_mode():
    ...
    # que disparar `update_highlight`, que por sua vez usará o novo `connections_mode`.
...
def update_highlight(cellid: int):
    ...
    """
    global current_hovered_info, mesh_esfera, plotter, face_centers # Acessa globais
...
def toggle_labels_callback():
    ...
    Alterna o valor da flag global `show_all_labels` (entre True e False).
    Em seguida, chama `update_labels` para que os rótulos sejam redesenhados
    de acordo com a nova configuração (mostrar todos ou apenas os destacados)
    e renderiza a cena para exibir a mudança.
    """
    ...
    if plotter and mesh_esfera and face_centers is not None:
        # Inverte o valor booleano da flag.
        show_all_labels = not show_all_labels
        logger.info(f"Alternando 'Mostrar Todos os Rótulos' para: {show_all_labels}")
        # Chama a função que efetivamente atualiza os rótulos na tela.
        update_labels(plotter, mesh_esfera, face_centers)
        # Redesenha a cena para que a mudança nos rótulos seja visível.
        plotter.render()
    else:
...
def main():
    ...
    num_subdivisions = 3         # Nível de detalhe (resolução) da esfera. Ajuste conforme necessário.
    esfera_radius = 5.0          # Raio da esfera no espaço 3D.
